...
# DESCRIPTION
# ----------------------------------------------------------------------------------------------------
## @file    mFileSystem/directoryLib.py @brief [ FILE   ] - Operate on directories.
## @package mFileSystem.directoryLib    @brief [ MODULE ] - Operate on directories.
...
class Directory(object):
    ...
    # ------------------------------------------------------------------------------------------------
    #
    ## @brief This method is used by mFileSystem.directoryLib.Directory.listDirectoriesRecursively method.
    #
    #  @param directory     [ str           | None | in  ] - Absolute path of a directory.
    ...
    #
    #  @exception mFileSystem.exceptionLib.DirectoryIsNotSet - If no directory has been set.
    #
    #  @return bool - Result.
    def hasBeenSet(self):

        if not self._directory:
            raise mFileSystem.exceptionLib.DirectoryIsNotSet('No directory has been set.')
    ...
    #
    #  @exception mFileSystem.exceptionLib.DirectoryAlreadyExists - If a directory with given newName already exists.
    #
    ...
    def rename(self, newName):
        ...
        if os.path.isdir(newDirectory):
            raise mFileSystem.exceptionLib.DirectoryAlreadyExists('Directory already exists, it could not be renamed: {}'.format(newDirectory))
    ...
    def toNativeSeparators(directory, startWithSeparator=True, endWithSeparator=False):
        ...
        if startWithSeparator:
            ...
            if not mCore.platformLib.Platform.isWindows():
    ...
    #
    #  mFileSystem.directoryLib.Directory class instance will be returned for the directory
    ...
    #  @exception N/A
    #
    #  @return mFileSystem.directoryLib.Directory - Instance of mFileSystem.directoryLib.Directory class.
    @staticmethod
    ...
    def listVersionedFolders(directory,
        ...
        if version == mFileSystem.versionLib.Version.kAll:
            return versionList

        elif version == mFileSystem.versionLib.Version.kLatest:
            return versionList[-1:][0]

        elif version == mFileSystem.versionLib.Version.kCurrent:
            return versionList[-1:][0]

        elif version == mFileSystem.versionLib.Version.kFirst:
            return versionList[:1][0]

        elif version == mFileSystem.versionLib.Version.kLast:
            return versionList[-1:][0]

        elif version == mFileSystem.versionLib.Version.kPrevious:
    ...
    #  @param version      [ enum, str, int | None  | in  ] - Requested version from mFileSystem.versionLib.Version class, string or int that would match with the version of the file.
